# Exercise 3 - Writing Unit Tests with Devin
...
- Understand how to use Devin's code editing capabilities for code modifications
...
Your task today is to use Devin to write high-quality unit tests that will:
...
## ✏️ Step 2: Using Devin to Generate One Additional Unit Test

To start off, you'd like to generate one additional unit test. We can use Devin's code editing capabilities to make an addition to the unit test suite.
...
4. Ask Devin to generate the test based on your comment. You can use a prompt like:

   ```
   Can you write a unit test based on the comment I just added? The test should verify that an empty cart message is displayed when the cart is empty.
   ```

5. Once the test is generated, try running it to make sure that it and the existing test pass. If there are any failures, try asking Devin how to fix them.

## 💭 Step 3: Using Devin to Generate Additional Unit Tests

There are many other tests that we can write for `CartPage`. We'll use Devin's autonomous editing capabilities to create comprehensive unit tests.

### Why Devin is Perfect for Unit Testing:
...
Devin automatically gathers context from several sources to understand your codebase:
...
### Instructions:
1. Ask Devin to generate tests for uncovered conditions in `CartPage`.
...
3. **Important**: Remember that while Devin is very powerful it is still a best practice to always review and validate the code generated by AI tools. Make sure to:
   - Check for correctness and completeness
   - Ensure adherence to your project's coding standards
   - Validate that all edge cases are covered

4. Run the newly-generated tests to ensure they pass and verify the expected behavior. If any tests fail you can always ask Devin for help fixing them.

### 🎁 Optional Task: Refine tests to handle an edge case that Devin didn't cover initially

**Pro Tip:** The more specific and contextual your prompts are, the better the generated code will be. Always review and iterate on AI-generated tests to ensure they meet your quality standards!

## 🎓 Step 4: Best Practices and Code Review

Now that you've generated comprehensive unit tests, it's important to ensure they follow industry best practices and maintain high quality. This step focuses on using Devin to review and refine your testing approach.
...
Let's use Devin to review and improve our test quality.

### Instructions:

Ask Devin to review your unit tests and make suggestions for improvement. Consider implementing its suggestions if you have time.
...
Great work! You've successfully used Devin's code editing capabilities to:
...
1. **How did Devin's autonomous editing compare to manual test writing?**
2. **What types of test scenarios did Devin excel at generating?**
3. **Where did you need to provide additional guidance or corrections?**
4. **How might you use Devin differently in future testing tasks?**

### Key Takeaways:
- Devin's code editing capabilities are powerful tools for generating and refining unit tests
...
In Exercise 4, we'll explore Devin's **autonomous capabilities** to tackle more complex, multi-file tasks like implementing the complete shopping cart feature with database integration, API endpoints, and frontend components.
